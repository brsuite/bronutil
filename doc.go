...
A Block defines a brocoin block that provides easier and more efficient
...
A Tx defines a brocoin transaction that provides more efficient manipulation of
...
The Address interface provides an abstraction for a Brocoin address.  While the
most common type is a pay-to-pubkey-hash, Brocoin already supports others and
...
To decode/encode an address:
	...
	addr, err := bronutil.DecodeAddress(addrString, defaultNet)
	if err != nil {
		fmt.Println(err)
		return
	}
