// Copyright (c) 2014-2016 The brsuite developers
...
//   https://github.com/brocoin/bips/blob/master/bip-0032.mediawiki
...
import (
	...
	"github.com/brsuite/bronutil/base58"
)
...
var masterKey = []byte("Brocoin seed")
...
func (k *ExtendedKey) pubKeyBytes() []byte {
	...
	// key if needed.
	if len(k.pubKey) == 0 {
		pkx, pky := bronec.S256().ScalarBaseMult(k.key)
		pubKey := bronec.PublicKey{Curve: bronec.S256(), X: pkx, Y: pky}
...
func (k *ExtendedKey) Child(i uint32) (*ExtendedKey, error) {
	...
	if ilNum.Cmp(bronec.S256().N) >= 0 || ilNum.Sign() == 0 {
	...
	if k.isPrivate {
		...
		ilx, ily := bronec.S256().ScalarBaseMult(il)
		...
		// public key.
		pubKey, err := bronec.ParsePubKey(k.key, bronec.S256())
		if err != nil {
			return nil, err
		}
		...
		//
		// childKey = serP(point(parse256(Il)) + parentKey)
		childX, childY := bronec.S256().Add(ilx, ily, pubKey.X, pubKey.Y)
		pk := bronec.PublicKey{Curve: bronec.S256(), X: childX, Y: childY}
		childKey = pk.SerializeCompressed()
	}

	// The fingerprint of the parent for the derived child is the first 4
	// bytes of the RIPEMD160(SHA256(parentPubKey)).
	parentFP := bronutil.Hash160(k.pubKeyBytes())[:4]
...
// ECPubKey converts the extended key to a bronec public key and returns it.
func (k *ExtendedKey) ECPubKey() (*bronec.PublicKey, error) {
	return bronec.ParsePubKey(k.pubKeyBytes(), bronec.S256())
}

// ECPrivKey converts the extended key to a bronec private key and returns it.
...
func (k *ExtendedKey) ECPrivKey() (*bronec.PrivateKey, error) {
	if !k.isPrivate {
		return nil, ErrNotPrivExtKey
	}

	privKey, _ := bronec.PrivKeyFromBytes(bronec.S256(), k.key)
	return privKey, nil
}

// Address converts the extended key to a standard brocoin pay-to-pubkey-hash
// address for the passed network.
func (k *ExtendedKey) Address(net *chaincfg.Params) (*bronutil.AddressPubKeyHash, error) {
	pkHash := bronutil.Hash160(k.pubKeyBytes())
	return bronutil.NewAddressPubKeyHash(pkHash, net)
}
...
func NewMaster(seed []byte, net *chaincfg.Params) (*ExtendedKey, error) {
	...
	//   I = HMAC-SHA512(Key = "Brocoin seed", Data = S)
	...
	if secretKeyNum.Cmp(bronec.S256().N) >= 0 || secretKeyNum.Sign() == 0 {
...
func NewKeyFromString(key string) (*ExtendedKey, error) {
	...
	if isPrivate {
		...
		if keyNum.Cmp(bronec.S256().N) >= 0 || keyNum.Sign() == 0 {
			return nil, ErrUnusableSeed
		}
	} else {
		// Ensure the public key parses correctly and is actually on the
		// secp256k1 curve.
		_, err := bronec.ParsePubKey(keyData, bronec.S256())
		if err != nil {
			return nil, err
		}
	}
