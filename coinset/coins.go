...
func (cs *CoinSet) TotalValue() (value bronutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue bronutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue bronutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount bronutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue bronutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue bronutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue bronutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue bronutil.Amount, coins []Coin) (Coins, error) {
...
// bronutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() bronutil.Amount {
	return bronutil.Amount(c.txOut().Value)
}
