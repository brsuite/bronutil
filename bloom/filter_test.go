...
import (
	...
	"github.com/brsuite/bronutil/bloom"
)
...
func TestFilterInsert(t *testing.T) {
	...
	err = f.MsgFilterLoad().BRONEncode(got, wire.ProtocolVersion, wire.LatestEncoding)
	if err != nil {
		t.Errorf("TestFilterInsert BRONDecode failed: %v\n", err)
		return
	}
...
func TestFilterFPRange(t *testing.T) {
	...
	for _, test := range tests {
		...
		err = f.MsgFilterLoad().BRONEncode(got, wire.ProtocolVersion, wire.LatestEncoding)
		if err != nil {
			t.Errorf("BRONDecode unexpected error: %v\n", err)
			continue
		}
		if !bytes.Equal(got.Bytes(), want) {
...
func TestFilterInsertWithTweak(t *testing.T) {
	...
	err = f.MsgFilterLoad().BRONEncode(got, wire.ProtocolVersion, wire.LatestEncoding)
	if err != nil {
		t.Errorf("TestFilterInsertWithTweak BRONDecode failed: %v\n", err)
		return
	}
...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := bronutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(bronutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := bronutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := bronutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
...
func TestFilterInsertP2PubKeyOnly(t *testing.T) {
	...
	block, err := bronutil.NewBlockFromBytes(blockBytes)
	if err != nil {
