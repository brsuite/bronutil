...
// PartialSig encapsulate a (BRON public key, ECDSA signature)
// pair, note that the fields are stored as byte slices, not
// bronec.PublicKey or bronec.Signature (because manipulations will
...
// Brocoin context (compressed/uncomp. OK).
func validatePubkey(pubKey []byte) bool {
	_, err := bronec.ParsePubKey(pubKey, bronec.S256())
	if err != nil {
		return false
	}
	return true
...
func validateSignature(sig []byte) bool {
	_, err := bronec.ParseDERSignature(sig, bronec.S256())
	if err != nil {
		return false
	}
	return true
